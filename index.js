...
app.get("/news/:newspaperId", async (req, res) => {
  ...
  const specificNews = newspapers.filter(
    (newspaper) => newspaper.name === newsId
  )[0].address;
  const specificBase = newspapers.filter(
    (newspaper) => newspaper.name === newsId
  )[0].base;

  axios.get(specificNews).then((response) => {
    const html = response.data;
    const $ = cheerio.load(html);
    const specArticles = [];
    $('a:contains("climate")', html).each(function () {
    ...
    res.json(spesArticles);
  });
});

app.listen(_port, () => console.log(`hello there from ${_port}!`));
